...
分类依据知识本身的技术属性：Agent 系统与工程、模型学习与训练、模型评测分别组织，后续可以增加新的领域。Agent Harness、运行时与框架不等于 Agent 算法；能力训练按其学习方法、奖励与优化机制归类，通用评测也不限定在 Agent 场景。

树的父子边表达主要归属与上下位关系，跨领域关系用 `related` 连接。每次归档检查相邻知识是否需要进一步分层；允许按主题增长拆分、迁移、合并分支，不固定顶层数量和层级深度，也不为每篇笔记新建分类。纯分类调整保留 ID、URL 与历史记录，不计为当天复学。具体经验和操作步骤见 [收工分类 SOP](../../.agents/skills/shougong/references/taxonomy.md)。
