...
- `/ideas/posts/<id>/`：内容决定版式；短 Idea 无大段导语，长文可有目录、引用、代码、callout、来源与补记。
...
- **补记某篇**：保留首次日期、追加观点变化、更新正文。

“收工”仍按已有 Skill 只处理 Knowledge。没有定时爬取、X API、后台模型或社交平台发帖；自动化由 workspace 内的 Codex 指令驱动。访问不了 X 时说明覆盖限制并转向可访问的一手渠道，不能假装已经跟踪。
